document.addEventListener('DOMContentLoaded', function () {
  ...
  function validateCiv() {
    ...
    resetError("civic");
    if (!/^\d{1,4}$/.test(civ)) {
      document.getElementById("civicError").textContent = "Inserisci un numero civico valido (massimo 4 cifre).";
      return false;
    }
    return true;
  }
  ...
  function validateExpiry() {
    ...
    const currentMonth = currentDate.getMonth() + 1; // mesi da 0 a 11
    const currentYear = currentDate.getFullYear() % 100; // ultimi due numeri

    if (parseInt(year) < currentYear || (parseInt(year) === currentYear && parseInt(month) < currentMonth)) {
      document.getElementById('expiryError').textContent = 'La carta è scaduta.';
      return false;
    }
    return true;
  }
  ...
  editBtn.addEventListener('click', function () {
    if (addressFields.style.display === 'block') {
      ...
      ['street', 'civic', 'zip', 'province'].forEach(function (id) {
        const el = document.getElementById(id);
        if (el) {
          el.value = initialAddressValues[id] || '';
          resetError(id);
        }
      });
    } else {
  ...
  const civInput = document.getElementById('civic');
  ...
  if (civInput) {
    civInput.addEventListener('keypress', function (e) {
      if (!/\d/.test(e.key)) e.preventDefault();
      if (civInput.value.length >= 4) e.preventDefault();
    });
    civInput.addEventListener('blur', validateCiv);
  }
